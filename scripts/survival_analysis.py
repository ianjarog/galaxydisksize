...
def emit_upperlimit_table(path):
    ...
    # Plain single-column tabular body only: the manuscript wraps it in a
    # one-column \begin{table} float (\scriptsize) with the caption on top and
    # the notes in \tablefoot, per the A&A camera-ready layout.
    L = [
        r"% Auto-generated by scripts/survival_analysis.py.",
        r"% Appendix table body (single-column tabular): all HCG members entering",
        r"% as beam-size upper limits (HI non-detections + beam-limited/unresolved",
        r"% members, marked $^{a}$). Wrapped in a table float by the manuscript.",
